...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('addCustomerOrderForm').addEventListener('submit', function(event) {
        ...
        var today = new Date();
        var orderDate = today.toISOString().slice(0, 10);
        
        // Calculate the receipt date (7 days from the order date)
        var receiptDateObj = new  Date(today.getTime() + 7 * 24 * 60 * 60 * 1000);
        var receiptDate = receiptDateObj.toISOString().slice(0, 10);      // Get the current date YYYY-MM-DD
        ...
        addCustomerOrder(customerId, orderList, orderDate, receiptDate, totalAmount);
    });
...
var addOrderProductContainer = document.querySelector('.admin-flex-button');
addOrderProductContainer.addEventListener('click', function(event) {
    ...
    if (event.target.classList.contains('add-order-product')) {
        ...
        newProductDiv.setAttribute('class', 'admin-inner-flex');
...
function resetOrderProductContainer() {
    productData = {};
    var parentElement = document.querySelector('.admin-flex');
...
var getProductElement = document.querySelector('.admin-flex');
let productData = {};
...
getProductElement.addEventListener('change', function(event) {
    if (event.target.name === 'addOrderProductDropdown') {
        ...
        var parentDiv = event.target.closest('.admin-inner-flex');
        var index = Array.prototype.indexOf.call(parentDiv.parentNode.children, parentDiv);

        // If productData for this index doesn't exist, create it
        if (!productData[index]) {
            productData[index] = {
