...
function addCustomerOrder(id, receiptDate, orderDate, totalAmount) {
    $.ajax({
        type: 'POST',
        url: 'addRecord.php',
        data: {id: id, receiptDate: receiptDate, orderDate: orderDate, totalAmount: totalAmount, typeOfFetch: 'addCustomerOrder'},
        success: function(response) {
            if (response.includes('successfully')) {
                document.getElementById('successModal').style.display = 'flex';
                document.getElementById('success-text').innerText = response;
